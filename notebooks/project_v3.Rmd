...
df_sub <- df_sub[df_sub$data >= "2020-10-01T00:00:00" & df_sub$data <= "2021-02-16T00:00:00",]
summary(df_sub)
...
var_mean <- compute_mean_points(df_sub$dimessi_guariti_per_day, 3, 3)
df_sub$mean_dimessi_guariti_per_day <- var_mean
...
#Nuovi positivi
var_mean <- compute_mean_points(df_sub$nuovi_positivi, 3, 3)
df_sub$mean_nuovi_positivi <- var_mean
...
summary(lm7_avg)
```
Non lo teniamo in considerazione perché ha lo stesso R^2 ma è più complicato il modello (ce lo aspettevamo perché logicamente non è significatico)
...
La dispersione con qp.glm2_avg è minore e il real residual deviance viene maggiore
...
```{r}
df_pred <- df[df$denominazione_regione == 'Lombardia',]
df_pred <- df_pred[df_pred$data >= "2021-01-25T00:00:00" & df_pred$data <= "2021-02-16T00:00:00",]

rt_lomb <- rt_lomb[rt_lomb$data >= "2021-01-25" & df_pred$data <= "2021-02-16",]

df_pred$rt<-rt_lomb$rt_positivi

df_pred <- df_pred[,c("data",
                    "totale_ospedalizzati",
                    "ricoverati_con_sintomi", 
                    "totale_positivi", 
                    "nuovi_positivi", 
...
dimessi_guariti_per_day <- diff(df_pred$dimessi_guariti)
df_pred$dimessi_guariti_per_day <- c(dimessi_guariti_per_day[1],dimessi_guariti_per_day)

deceduti_per_day <- diff(df_pred$deceduti)
df_pred$deceduti_per_day <- c(deceduti_per_day[1],deceduti_per_day)

#colnames(df_pred)[colnames(df_pred)=="nuovi_positivi"] <- "mean_nuovi_positivi"
#colnames(df_pred)[colnames(df_pred)=="dimessi_guariti_per_day"] <- "mean_dimessi_guariti_per_day"

df_pred$mean_dimessi_guariti_per_day <- compute_mean_points(df_pred$nuovi_positivi, 3, 3)
df_pred$mean_nuovi_positivi <- compute_mean_points(df_pred$dimessi_guariti_per_day, 3, 3)

df_pred <- df_pred[df_pred$data >= "2021-02-01T00:00:00" & df_pred$data <= "2021-02-16T00:00:00",]

df1<-merge(df_sub, df_pred, all=TRUE)

df_tot_osp<-c(df_sub$totale_ospedalizzati, df_pred$totale_ospedalizzati)
data_plot<-c(df_sub$data, df_pred$data)
pred<-data.frame(data_plot, df_tot_osp)

colnames(pred)[colnames(pred)=="df_tot_osp"] <- "totale_ospedalizzati"
colnames(pred)[colnames(pred)=="data_plot"] <- "data"
...
```{r, echo=FALSE}
lm4_pred<-predict(lm4_avg, newdata = df1, interval = "prediction", level=0.95)

lm4_pred

df1$lm4_pred<-as.numeric(lm4_pred[,'fit'])

mse_lm4<-mean((df_pred$totale_ospedalizzati - as.numeric(lm4_pred[,'fit']))^2)

pred_lm4<-c(df_sub$totale_ospedalizzati, as.numeric(lm4_pred[,'fit']) )
pred$pred_lm4<-pred_lm4

# pred$pred_lm4_lwr <- as.numeric(lm4_pred[,'lwr'])

```

```{r, echo=FALSE}
ggplot(df1, aes(x=as.Date(data), y=lm4_pred, col='red') ) +
  geom_line() +
  xlab("Date") +
  scale_y_continuous(breaks = seq(0,9500,by=1000))+
  geom_line(aes(x=as.Date(data), y=totale_ospedalizzati, col='black')) 
    # geom_point(lm4_pred, aes(y=lwr))
...
pred_glm1<-c(df_sub$totale_ospedalizzati, as.numeric(glm1_avg_pred) )
pred$pred_glm1<-pred_glm1
```

```{r, echo=FALSE}
ggplot(pred, aes(x=as.Date(data), y=pred_glm1, col='red') ) +
  geom_line() +
  xlab("Date") +
  scale_y_continuous(breaks = seq(0,9500,by=1000))+
  geom_line(aes(x=as.Date(data), y=totale_ospedalizzati, col='black')) 
...
pred_qp.glm2<-c(df_sub$totale_ospedalizzati, as.numeric(qp.glm2_avg_pred) )
pred$pred_qp.glm2<-pred_qp.glm2

```

```{r, echo=FALSE}
ggplot(pred, aes(x=as.Date(data), y=pred_qp.glm2, col='red') ) +
  geom_line() +
  xlab("Date") +
  scale_y_continuous(breaks = seq(0,9500,by=1000))+
  geom_line(aes(x=as.Date(data), y=totale_ospedalizzati, col='black')) 
...
pred_gamma.glm2<-c(df_sub$totale_ospedalizzati, as.numeric(gamma.glm2_avg_pred) )
pred$pred_gamma.glm2<-pred_gamma.glm2

```

```{r, echo=FALSE}
ggplot(pred, aes(x=as.Date(data), y=pred_gamma.glm2, col='red') ) +
  geom_line() +
  xlab("Date") +
  scale_y_continuous(breaks = seq(0,9500,by=1000))+
  geom_line(aes(x=as.Date(data), y=totale_ospedalizzati, col='black')) 
...
[12:25] INSOGNA VALERIA [SM3500504]
## GAM con Gamma log link{r, echo=FALSE}
...
gammma.gam1_avg_pred

#mse_gamma.glm2<-mean((df_pred$totale_ospedalizzati - as.numeric(gamma.glm2_avg_pred))^2)

pred_gammma.gam1<-c(df_sub$totale_ospedalizzati, as.numeric(gammma.gam1_avg_pred))
pred$pred_gammma.gam1<-pred_gammma.gam1
```

```{r, echo=FALSE}
ggplot(pred, aes(x=as.Date(data), y=pred_gammma.gam1, col='red') ) +
  geom_line() +
  xlab("Date") +
  scale_y_continuous(breaks = seq(0,9500,by=1000))+
  geom_line(aes(x=as.Date(data), y=totale_ospedalizzati, col='black')) 
...
pred_qp.glm2_avg.rt<-c(df_sub$totale_ospedalizzati, as.numeric(qp.glm2_avg.rt_pred))
pred$pred_qp.glm2_avg.rt<-pred_qp.glm2_avg.rt

```
```{r}
ggplot(pred, aes(x=as.Date(data), y=pred_qp.glm2_avg.rt, col='red') ) +
  geom_line() +
  xlab("Date") +
  scale_y_continuous(breaks = seq(0,9500,by=1000))+
  geom_line(aes(x=as.Date(data), y=totale_ospedalizzati, col='black'))+
  geom_line(data=df1,aes(x=as.Date(data),y=mean_dimessi_guariti_per_day, color="blue")) +
  geom_line(data=df1,aes(x=as.Date(data),y=deceduti_per_day, color="green")) +
  geom_line(data=df1,aes(x=as.Date(data),y=mean_nuovi_positivi, color="pink")) 
    # geom_errorbar(aes(ymin = lower, ymax = upper))
